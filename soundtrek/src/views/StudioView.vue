...
const studioName = computed(
  () =>
    studioSoundtracks.value[0]?.studio ??
...
useHead(computed(() => ({
  ...
  meta: [
    { name: "description", content: `Video game soundtracks by ${studioName.value} on SoundTrek.` },
    { property: "og:title", content: `${studioName.value} | SoundTrek` },
    { property: "og:description", content: `Video game soundtracks by ${studioName.value} on SoundTrek.` },
    { property: "og:url", content: `https://soundtrek.app/studio/${slug.value}` },
  ],
})));
</script>
  ...
  <div class="page">
    ...
    <template v-else>
      <div class="page-inner">
        <PageHero label="Studio" :title="studioName" :subtitle="subtitle" />

        <div v-if="studioSoundtracks.length === 0" class="state">
          <p>No soundtracks found for this studio.</p>
          <RouterLink to="/" class="home-link">Back to discovery</RouterLink>
        </div>

        <section v-else class="grid">
          <RouterLink
            v-for="s in studioSoundtracks"
            :key="s.id"
            :to="`/soundtrack/${s.id}`"
            class="grid-card"
          >
