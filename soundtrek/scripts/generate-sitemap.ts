...
async function main() {
  ...
  const soundtrackUrls = (soundtracks ?? []).map((s) =>
    url(`${BASE}/soundtrack/${s.id}`, s.updated_at?.split('T')[0]),
  )

  const composerSlugs = [...new Set((soundtracks ?? []).map((s) => toSlug(s.composer)))]
  const composerUrls = composerSlugs.map((slug) => url(`${BASE}/composer/${slug}`))
