 ...
 * profile image (Wikipedia lead image, falling back to the Wikidata P18
 * portrait), and upserts both into the composers table.
 *
 * Usage:
...
async function fetchWikiSummary(pageTitle: string): Promise<WikiSummary> {
  await sleep(300);
  try {
    const res = await fetch(
    ...
    // The summary thumbnail is ~320px; Commons thumb URLs embed the width,
    // so bump it for a sharper avatar. Falls through untouched if the URL
    // doesn't match the thumb pattern.
    const thumb = data.thumbnail?.source ?? null;
    const imageUrl = thumb ? thumb.replace(/\/(\d+)px-/, "/512px-") : null;

    return { bio: data.extract ?? null, imageUrl };
  } catch {
    return { bio: null, imageUrl: null };
  }
}

// Wikidata P18 ("image") — the curated portrait claim. Used when the
// Wikipedia page has no lead image.
async function fetchWikidataImage(enwikiTitle: string): Promise<string | null> {
  await sleep(300);
  try {
    const res = await fetch(
      `https://www.wikidata.org/w/api.php?action=wbgetentities&sites=enwiki&titles=${encodeURIComponent(enwikiTitle)}&props=claims&format=json`,
      { headers: { "User-Agent": "SoundTrek/1.0 (https://soundtrek.app)" } },
    );
    if (!res.ok) return null;
    const data = (await res.json()) as {
...
async function main() {
  ...
  const allNames = [
    ...new Set(
      (soundtracks ?? []).flatMap(
        (s: { composers?: string[] }) => s.composers ?? [],
      ),
    ),
  ].sort() as string[];
  ...
  const bySlug = new Map(
    (existing ?? []).map(
      (c: { slug: string; bio: string | null; image_url: string | null }) => [
        c.slug,
        c,
      ],
    ),
  ...
  const toProcess = allNames
    .filter((name) => {
      ...
      const c = bySlug.get(toSlug(name));
      return !c || !c.bio || !c.image_url;
    })
    .slice(0, LIMIT);

  console.log(
    `Unique composers: ${allNames.length} | Need bio or image: ${toProcess.length}\n`,
  );
  ...
  for (const name of toProcess) {
    ...
    const { bio, imageUrl: wikiImage } = await fetchWikiSummary(wikiTitle);
    if (!bio) {
    ...
    if (!imageUrl) {
      process.stdout.write("  No lead image — trying Wikidata P18...");
      imageUrl = await fetchWikidataImage(wikiTitle);
      console.log(imageUrl ? " found" : " none");
    }
