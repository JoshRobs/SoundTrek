...
// .wrangler/tmp/bundle-bR1dyg/strip-cf-connecting-ip-header.js
...
async function igdbSearch(query, env) {
  ...
  async function runQuery(body) {
    ...
    if (!r.ok)
      return { error: `IGDB request failed: ${r.status}` };
    return await r.json();
  }
...
function hasUnexpectedSeriesNumber(gt, vt) {
  ...
  if (arabicMatch) {
    ...
    if (!gameNumbers.includes(arabicMatch[1]))
      return true;
  }
  ...
  if (romanMatch && ROMAN_NUMERALS.has(romanMatch[1])) {
    ...
    if (!gameRomans.includes(romanMatch[1]))
      return true;
  }
  return false;
}
...
function scoreVideo(videoTitle, gameTitle, durationSeconds) {
  ...
  if (durationSeconds < MIN_DURATION)
    return -1;
  if (Math.abs(durationSeconds - 3600) <= 2)
    return -1;
  if (BAD_KEYWORDS.some((b) => vt.includes(b)))
    return -1;
  let score = 0;
  if (vt === gt)
    score += 5;
    ...
    return -1;
  if (hasUnexpectedSeriesNumber(gt, vt))
    return -1;
  if (!vt.includes("soundtrack") && !vt.includes(" ost") && !vt.includes("(ost)") && !vt.includes("[ost]"))
    return -1;
  if (OST_KEYWORDS.some((k) => vt.includes(k)))
    score += 3;
  else
    score += 1;
...
// .wrangler/tmp/bundle-bR1dyg/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-bR1dyg/middleware-loader.entry.ts
